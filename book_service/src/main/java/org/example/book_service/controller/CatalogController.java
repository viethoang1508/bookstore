...
public class CatalogController {
    ...
    @GetMapping
    public ResponseEntity<BaseResponse<Page<BookSummaryDTO>>> getBooks(@RequestBody BookSearchRequest request){
        return ResponseEntity.ok(new BaseResponse<>(catalogService.getBooks(request), "Get catalog successfully"));
    }

    @GetMapping("/id")
    public ResponseEntity<BaseResponse<BookSummaryDTO>> getBookById(@RequestParam String id){
        return ResponseEntity.ok(new BaseResponse<>(catalogService.getBookById(id), "Get book information successfully"));
    }

    @GetMapping("/categories/id")
    public ResponseEntity<BaseResponse<Page<BookSummaryDTO>>> getBooksByCategory(@RequestParam String categoryId){
        return ResponseEntity.ok(new BaseResponse<>(catalogService.getBooksByCategory(categoryId), "Get all books in this category successfully"));
    }
